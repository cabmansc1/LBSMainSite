...
// Banner photo — look for a dedicated banner upload
$bannerPhoto = null;
foreach ($business['photos'] ?? [] as $p) {
    if (($p['photo_type'] ?? '') === 'banner') {
        $bannerPhoto = $p;
        break;
    }
}
...
<head>
    ...
    <style>
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .biz-header {
            background: white; border-radius: 0 0 16px 16px; padding: 30px; padding-top: 70px;
            box-shadow: 0 4px 20px rgba(0,0,0,.04); border: 1px solid #e2e8f0; border-top: none; margin-bottom: 24px; position: relative;
        }
        ...
        .btn-action { background: #38b6ff; color: white; padding: 12px 24px; border: none; border-radius: 10px; font-weight: 600; font-size: 0.95rem; display: inline-block; transition: all 0.25s ease; }
        .btn-action:hover { background: #0ea5e9; transform: translateY(-2px); box-shadow: 0 8px 24px rgba(56,182,255,.3); color: white; }
        .btn-secondary { background: #64748b; }
        .btn-secondary:hover { background: #374151; box-shadow: 0 8px 24px rgba(100,116,139,.2); }

        /* Social Media Icons */
        .social-links { display: flex; gap: 12px; margin-top: 16px; flex-wrap: wrap; }
        .social-link { display: inline-flex; align-items: center; justify-content: center; width: 40px; height: 40px; border-radius: 10px; background: #f8fafc; border: 1px solid #e2e8f0; transition: all 0.25s ease; }
        .social-link:hover { background: #38b6ff; border-color: #38b6ff; }
        .social-link svg { width: 20px; height: 20px; fill: #38b6ff; transition: fill 0.2s; }
        .social-link:hover svg { fill: white; }

        /* Content Sections */
        .content-section { background: white; border-radius: 16px; padding: 28px; box-shadow: 0 4px 20px rgba(0,0,0,.04); border: 1px solid #e2e8f0; margin-bottom: 24px; }
        ...
        .photo-item { border-radius: 12px; overflow: hidden; background: #f1f5f9; cursor: pointer; transition: opacity 0.2s; }
        ...
        .sidebar-section { background: white; border-radius: 16px; padding: 28px; box-shadow: 0 4px 20px rgba(0,0,0,.04); border: 1px solid #e2e8f0; }
        ...
        .form-group input, .form-group textarea {
            ...
            font-size: 15px; font-family: inherit; transition: border-color 0.25s ease, box-shadow 0.25s ease;
        }
        .form-group input:focus, .form-group textarea:focus { outline: none; border-color: #38b6ff; box-shadow: 0 0 0 4px rgba(56,182,255,0.1); }
        .form-group textarea { resize: vertical; min-height: 90px; }

        .special-offer { background: linear-gradient(135deg, #fef3c7, #fde68a); border: 2px dashed #f59e0b; border-radius: 14px; padding: 16px; text-align: center; margin-bottom: 12px; }
        ...
        /* Claim Listing */
        .claim-listing-box { background: linear-gradient(135deg, #f0f9ff, #e0f2fe); border: 2px dashed #38b6ff; border-radius: 14px; padding: 20px; }
        ...
        .related-card:hover { transform: translateY(-3px); box-shadow: 0 12px 32px rgba(56,182,255,.12); border-color: #38b6ff; }
    ...
    <div class="banner-header">
        <?php if ($bannerPhoto): ?>
            <img src="<?= htmlspecialchars($bannerPhoto['url']) ?>"
                 ...
                 width="<?= !empty($bannerPhoto['width']) ? (int)$bannerPhoto['width'] : 1200 ?>" height="<?= !empty($bannerPhoto['height']) ? (int)$bannerPhoto['height'] : 240 ?>"
                 fetchpriority="high">
        <?php else: ?>
                ...
                <div class="biz-logo">
                    <?php if ($primaryPhoto): ?>
                        <img src="<?= htmlspecialchars($primaryPhoto['medium_url'] ?? $primaryPhoto['url']) ?>"
                             ...
                             width="<?= !empty($primaryPhoto['medium_width']) ? (int)$primaryPhoto['medium_width'] : 120 ?>" height="<?= !empty($primaryPhoto['medium_height']) ? (int)$primaryPhoto['medium_height'] : 120 ?>">
                    <?php else: ?>
                        <?= $icon ?>
                    <?php endif; ?>
                </div>
                        ...
                        <?php foreach ($galleryPhotos as $idx => $photo): ?>
                            <div class="photo-item" onclick="openLightbox(<?= $idx ?>)">
                                <img src="<?= htmlspecialchars($photo['medium_url'] ?? $photo['url']) ?>"
                                     ...
                                     width="<?= !empty($photo['medium_width']) ? (int)$photo['medium_width'] : 600 ?>" height="<?= !empty($photo['medium_height']) ? (int)$photo['medium_height'] : 450 ?>"
                                     loading="lazy">
                            </div>
                        <?php endforeach; ?>
                    </div>
