...
return [
    ...
    /*
     * category.php had an entry here until it was deleted.
     *
     * It was the category browser before /directory/category/{slug},
     * and was held at noindex while it stayed on disk — a live file
     * with an unknown inbound link is not worth removing blind. That
     * link has since been checked and the file removed, so the entry
     * went with it: this config is keyed by filename, and an entry for
     * a file that does not exist is never read.
     *
     * What replaced it is a 301 in both layers — .htaccess for the PHP
     * side, next.config.ts for the app — carrying ?category={slug}
     * across to the matching category page rather than dumping every
     * old link at the directory root.
     */
